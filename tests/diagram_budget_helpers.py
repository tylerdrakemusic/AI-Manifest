...
def measure_source(path: Path) -> DiagramMetrics:
    ...
    edge_count = sum(
        1 for line in lines if any(operator in line for operator in ("-->", "==>", "-.->", "---", "===", "}|"))
    )
...
def validate_diagram(spec: DiagramSpec) -> ValidationResult:
    ...
    for code, value, limit in (("utf8_characters", metrics.utf8_characters, budget.max_utf8_characters), ("utf8_bytes", metrics.utf8_bytes, budget.max_utf8_bytes), ("nodes", metrics.nodes, budget.max_nodes), ("edges", metrics.edges, budget.max_edges)):
        if value > limit:
            findings.append(Finding(code, f"{code}={value} exceeds budget {limit}"))
    for code, value, limit in (("renderer_url_risk", metrics.renderer_url_risk, budget.max_renderer_url_risk), ("fallback_risk", metrics.fallback_risk, budget.max_fallback_risk)):
