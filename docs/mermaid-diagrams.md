...
- `manifest-derived-portal-and-image.mmd` is derived from the architecture overview.
- `manifest-derived-todo-and-backup.mmd` is derived from the architecture overview.

The three derived views retain their `Traceability.parent` markers, and the
architecture overview lists all derived paths. The shared Workspace inventory
...
the six-source set, UTF-8 readability, derived-view lineage, and the shared
diagram budget contract.